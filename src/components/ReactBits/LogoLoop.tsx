  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      const easingFactor = 1 -
        Math.exp(-deltaTime / ANIMATION_CONFIG.SMOOTH_TAU);
      ...
      if (isVertical) {
        ...
        if (sequenceHeight > 0) {
          ...
          const viewport = containerRef.current?.clientHeight ?? parentHeight ??
            sequenceHeight;
          const copiesNeeded = Math.ceil(viewport / sequenceHeight) +
            ANIMATION_CONFIG.COPY_HEADROOM;
          setCopyCount(Math.max(ANIMATION_CONFIG.MIN_COPIES, copiesNeeded));
        }
      } else if (sequenceWidth > 0) {
        setSeqWidth(Math.ceil(sequenceWidth));
        const viewport = containerWidth ||
          (typeof window !== "undefined" ? window.innerWidth : sequenceWidth);
        const copiesNeeded = Math.ceil(viewport / sequenceWidth) +
          ANIMATION_CONFIG.COPY_HEADROOM;
        setCopyCount(Math.max(ANIMATION_CONFIG.MIN_COPIES, copiesNeeded));
      }
    }, [isVertical]);
